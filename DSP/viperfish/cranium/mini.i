...
struct Network
{
    ...
    void fit(Optimizer * opt, const SimpleEigen::Matrix<double>& x, const SimpleEigen::Matrix<double> & y, int batch_size, int epoch, int seed=-1)
    {        
        Eigen::MatrixXd _x(x.rows(),x.cols()),_y(y.rows(),y.cols());
        ...
        try {
            net->fit(*opt->opt,_x, _y, batch_size, epoch, seed);
        } 
    ...
    SimpleEigen::Matrix<double> predict(SimpleEigen::Matrix<double>& x)
    {
        Eigen::MatrixXd _x(x.rows(),x.cols());
        for(size_t i = 0; i < x.rows(); i++)
            for(size_t j = 0; j < x.cols(); j++)
                _x(i,j) = x.matrix(i,j);

        Matrix<double> r;
        Eigen::MatrixXd ret = net->predict(_x);
        r.resize(ret.rows(),ret.cols());
        for(size_t i = 0; i < ret.rows(); i++)
            for(size_t j = 0; j < ret.cols(); j++)
                r.matrix(i,j) = ret(i,j);
        return r;
    }
