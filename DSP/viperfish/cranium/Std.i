...
%template(StringVector) Std::StdVector<Std::StdString>;
%template(FloatVector)  Std::StdVector<float>;
%template(DoubleVector)  Std::StdVector<double>;
%template(Int8Vector)  Std::StdVector<int8_t>;
%template(UInt8Vector)  Std::StdVector<uint8_t>;
%template(Int16Vector)  Std::StdVector<int16_t>;
%template(UInt16Vector)  Std::StdVector<uint16_t>;
%template(Int32Vector)  Std::StdVector<int32_t>;
%template(UInt32Vector)  Std::StdVector<uint32_t>;
%template(Int64Vector)  Std::StdVector<i64_t>;
%template(UInt64Vector)  Std::StdVector<u64_t>;

%template(StringMatrix) Std::StdMatrix<Std::StdString>;
%template(FloatMatrix)  Std::StdMatrix<float>;
%template(DoubleMatrix)  Std::StdMatrix<double>;
%template(Int8Matrix)  Std::StdMatrix<int8_t>;
%template(UInt8Matrix)  Std::StdMatrix<uint8_t>;
%template(Int16Matrix)  Std::StdMatrix<int16_t>;
%template(UInt16Matrix)  Std::StdMatrix<uint16_t>;
%template(Int32Matrix)  Std::StdMatrix<int32_t>;
%template(UInt32Matrix)  Std::StdMatrix<uint32_t>;
%template(Int64Matrix)  Std::StdMatrix<i64_t>;
%template(UInt64Matrix)  Std::StdMatrix<u64_t>;

// FloatMatrix = Intel Mkl
// DblMatrix   = Intel Mkl
// CpxMatrix   = Intel Mkl
// DblCpxMatrix= Intel MKL
// Int32Matrix = Intel MKL
// Int64Matrix = Intel MKL
...
%inline %{
    ...
    static int busy_handler_cb(void * db);
    ...
    struct Database 
    {
       ...
       // int executef(char const* sql, ...);
        ...
        void set_update_handler(SWIGLUA_FN h)
        {
            update_handler = h;
        }
    ...
    struct Statement
    {
        ...
        int ibind_int(int idx, int value)
        {
            return p_st->bind(idx,value);
        }
        
        int ibind_double(int idx, double value)
        {
            return p_st->bind(idx,value);
        }
        int ibind_i64(int idx, long long int value)
        {
            return p_st->bind(idx,value);
        }
        int ibind_semantic(int idx, char const* value, copy_semantic fcopy) {
            return p_st->bind(idx,value,fcopy);
        }
        int ibind_semantic_n(int idx, void const* value, int n, copy_semantic fcopy) {
            return p_st->bind(idx,value,n,fcopy);
        }
        int ibind_value(int idx, std::string const& value, copy_semantic fcopy) {
        ...
        int ibind_null(int idx)
        {
            null_type null;
            return p_st->bind(idx,null);
        }
        int bind_int(char const* name, int value)
        {
            return p_st->bind(name,value);
        }
        int bind_double(char const* name, double value)
        {
            return p_st->bind(name,value);
        }
        int bind_i64(char const* name, long long int value)
        {
            return p_st->bind(name,value);
        }
        int bind_semantic(char const* name, char const* value, copy_semantic fcopy) {
            return p_st->bind(name,value,fcopy);
        }
        int bind_semantic_n(char const* name, void const* value, int n, copy_semantic fcopy)
        {
            return p_st->bind(name,value,fcopy);
        }
        int bind_value(char const* name, std::string const& value, copy_semantic fcopy) {
            return p_st->bind(name,value,fcopy);
        }
