std::vector<float> FFTW_Convolution(std::vector<float> & x, std::vector<float> & y)
    int M = x.size();
    int N = y.size();
    float x[M] = {0,1,0,0};
...
std::vector<float> FFTW_Deconvolution(std::vector<float> & x, std::vector<float> & y)
    int M = x.size();
    int N = y.size();
    float x[M] = {0,1,0,0};
    float y[N] = {0,0,1,0,0,0,0,0};
    ...
    for( int idx = 0; idx < M+N-1; idx++ ) {
        ...
        if(std::abs(out_b[i]) != 0.0f)
            in_rev[idx] = out_a[i] / out_b[i];
        else 
            in_rev[idx] = std::complex<float>(0.0f,0.0f);
    }
