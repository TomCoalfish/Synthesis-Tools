...
struct Chorus {
    ...
    void SetMaxDelay(float maxDelayTime) {
        delayBufferSamples = (int)(maxDelayTime/1000.0f * sampleRate) + 1;
        if (delayBufferSamples < 1)delayBufferSamples = 1;        
        delayBuffer.resize(delayBufferSamples, numChannels);
    }
    void SetChannels(int n) { num_channel = n; }
    void SetVoices(int n) { num_voices = n; }
    void SetDelay(float ms) { current_delay = ms/1000.0f; }
    void SetWidth(float ms) { current_width = ms/1000.0f; }
    void SetFrequency(float f) { current_frequency = f; }
