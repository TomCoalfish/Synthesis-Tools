...
public:
  ...
  unsigned long getMaximumDelay( void ) { return this->template inputs_.size() - 1; };
  ...
  */
  ...
  T lastOut( void ) const { return this->template lastFrame_[0]; };
...
inline T DelayL<T>::nextOut( void )
{
  if ( doNextOut_ ) {
    ...
    nextOutput_ = this->template inputs_[outPoint_] * omAlpha_;
    // Second 1/2 of interpolation
    if (outPoint_+1 < this->template inputs_.size())
      nextOutput_ += this->template inputs_[outPoint_+1] * alpha_;
    else
      nextOutput_ += this->template inputs_[0] * alpha_;
    doNextOut_ = false;
  }
...
{
  if ( delay + 1 > this->template inputs_.size() ) { // The value is too big.
    this->template oStream_ << "DelayL::setDelay: argument (" << delay << ") greater than  maximum!";
    this->handleError( StkError::WARNING ); return;
  }

  if (delay < 0 ) {
    this->template oStream_ << "DelayL::setDelay: argument (" << delay << ") less than zero!";
    this->handleError( StkError::WARNING ); return;
  }
  ...
  while ( outPointer < 0 )
    outPointer += this->template inputs_.size(); // modulo maximum length
  ...
  if ( outPoint_ == this->template inputs_.size() ) outPoint_ = 0;
  doNextOut_ = true;
}
...
{
  this->template inputs_[inPoint_++] = input * this->template gain_;

  // Increment input pointer modulo length.
  if ( inPoint_ == this->template inputs_.size() )
    inPoint_ = 0;

  this->template lastFrame_[0] = nextOut();
  doNextOut_ = true;

  // Increment output pointer modulo length.
  if ( ++outPoint_ == this->template inputs_.size() )
    outPoint_ = 0;

  return this->template lastFrame_[0];
}
...
inline StkFrames<T>& DelayL<T>::tick( StkFrames<T>& frames, unsigned int channel )
{
  ...
  for ( unsigned int i=0; i<frames.frames(); i++, samples += hop ) {
    this->template inputs_[inPoint_++] = *samples * this->template gain_;
    if ( inPoint_ == this->template inputs_.size() ) inPoint_ = 0;
    *samples = nextOut();
    doNextOut_ = true;
    if ( ++outPoint_ == this->template inputs_.size() ) outPoint_ = 0;
  }

  this->template lastFrame_[0] = *(samples-hop);
  return frames;
}
...
inline StkFrames<T>& DelayL<T>::tick( StkFrames<T>& iFrames, StkFrames<T>& oFrames, unsigned int iChannel, unsigned int oChannel )
{
  ...
  for ( unsigned int i=0; i<iFrames.frames(); i++, iSamples += iHop, oSamples += oHop ) {
    this->template inputs_[inPoint_++] = *iSamples * this->template gain_;
    if ( inPoint_ == this->template inputs_.size() ) inPoint_ = 0;
    *oSamples = nextOut();
    doNextOut_ = true;
    if ( ++outPoint_ == this->template inputs_.size() ) outPoint_ = 0;
  ...
  this->template lastFrame_[0] = *(oSamples-oHop);
  return iFrames;
}
...
{
  if ( delay < 0.0 ) {
    this->template oStream_ << "DelayL::DelayL: delay must be >= 0.0!";
    this->handleError( StkError::FUNCTION_ARGUMENT );
  }

  if ( delay > (T) maxDelay ) {
    this->template oStream_ << "DelayL::DelayL: maxDelay must be > than delay argument!";
    this->handleError( StkError::FUNCTION_ARGUMENT );
  }

  // Writing before reading allows delays from 0 to length-1. 
  if ( maxDelay + 1 > this->template inputs_.size() )
    this->template inputs_.resize( maxDelay + 1, 1, 0.0 );
...
{
  if ( delay < this->template inputs_.size() ) return;
  this->template inputs_.resize(delay + 1, 1, 0.0);
}
...
T DelayL<T>::tapOut( unsigned long tapDelay )
{
  ...
  while ( tap < 0 ) // Check for wraparound.
    tap += this->template inputs_.size();

  return this->template inputs_[tap];
}
...
void DelayL<T>::tapIn( T value, unsigned long tapDelay )
{
  ...
  this->template inputs_[tap] = value;
}
